...
import type {
    LiquidButtonHandle,   LiquidButtonEventMap,   LiquidButtonOptions,
    LiquidSwitchHandle,   LiquidSwitchEventMap,   LiquidSwitchOptions,
    LiquidSliderHandle,   LiquidSliderEventMap,   LiquidSliderOptions,
    LiquidCursorHandle,   LiquidCursorEventMap,   LiquidCursorOptions,
    LiquidInputHandle,    LiquidInputEventMap,    LiquidInputOptions,
    LiquidDialHandle,     LiquidDialEventMap,     LiquidDialOptions,
    LiquidTooltipHandle,  LiquidTooltipEventMap,  LiquidTooltipOptions,
    LiquidProgressHandle, LiquidProgressEventMap, LiquidProgressOptions,
    LiquidGlassHandle,
} from './types.js';
...
function makeCleanupTracker() {
    ...
    function add(target: EventTarget, event: string, fn: EventListenerOrEventListenerObject, opts?: AddEventListenerOptions) {
    ...
    implements LiquidButtonHandle
{
    ...
    setLabel(text: string): this { this._labelSpan.textContent = text; return this; }
    destroy(): void { this._cleanup(); this._emit('destroy'); }
}
...
export function createLiquidButton(
    ...
    const glass = createLiquidGlass(el, {
        bezelWidth: 26, glassThickness: 120, refractiveIndex: 2.0,
        blur: 0.8, saturation: 1.2, specularSlope: 0.8, ...glassOpts,
    }) as InternalGlass;
    ...
    let hover = false, pressed = false, af: number | null = null;
    ...
    function kick() { if (!af) af = requestAnimationFrame(loop); }
    ...
    ev.add(el, 'mouseenter', (e) => { hover = true;  handle._emit('mouseenter', e as MouseEvent); kick(); });
    ev.add(el, 'mouseleave', (e) => { hover = false; pressed = false; handle._emit('mouseleave', e as MouseEvent); kick(); });
    ev.add(el, 'mousedown',  (e) => { pressed = true;  handle._emit('mousedown',  e as MouseEvent); kick(); });
    ev.add(el, 'mouseup',    (e) => { pressed = false; handle._emit('mouseup',    e as MouseEvent); kick(); });
    ev.add(el, 'touchstart', () => { pressed = true;  kick(); }, { passive: false });
    ev.add(el, 'touchend',   () => { pressed = false; kick(); });
    ...
    implements LiquidSwitchHandle
{
    ...
    get checked(): boolean { return this._state.chk; }
    set checked(v: boolean) { this._setChecked(!!v, false); }

    toggle(): this { this._setChecked(!this._state.chk, true); return this; }
    destroy(): void { this._cleanup(); this._emit('destroy'); }
}
...
export function createLiquidSwitch(
    ...
    const glass = createLiquidGlass(thumb, {
        ...
        bezelWidth: BW, glassThickness: 47, refractiveIndex: 1.5,
        blur: 0.2, saturation: 6, specularSlope: 0.5, filterMode: 'composite',
        ...glassOpts,
    }) as InternalGlass;
    ...
    function loop() {
        ...
        const xr = sp.xr.update(DT), sc = sp.sc.update(DT),
              bo = sp.bo.update(DT), tc = sp.tc.update(DT);
    ...
    ev.add(thumb, 'mousedown',  (e: Event) => { state.pd = true; state.ix = (e as MouseEvent).clientX; state.xr = state.chk ? 1 : 0; kick(); });
    ev.add(thumb, 'touchstart', (e: Event) => { e.preventDefault(); state.pd = true; state.ix = (e as TouchEvent).touches[0].clientX; state.xr = state.chk ? 1 : 0; kick(); }, { passive: false });
    ev.add(window, 'mousemove', (e: Event) => {
        if (!state.pd) return;
        const cx = (e as MouseEvent).clientX;
        const rv = (state.chk ? 1 : 0) + (cx - state.ix) / tr;
        state.xr = Math.min(1, Math.max(0, rv)) + ((rv < 0 ? 1 : -1) * (rv < 0 ? -rv : rv > 1 ? rv - 1 : 0)) / 22;
        sp.xr.setTarget(state.xr); kick();
    });
    ev.add(window, 'touchmove', (e: Event) => {
        if (!state.pd) return;
        e.preventDefault();
    ...
    ev.add(window, 'mouseup',   (e: Event) => {
        ...
        if (next !== state.chk) setChecked(next, true); else kick();
    });
    ...
    ev.add(track, 'click', (e: Event) => { if (e.target === track) setChecked(!state.chk, true); });
    ...
    implements LiquidSliderHandle
{
    ...
    set value(v: number) { this._setVal(v); }
    /** @internal */ _updateCurrent(v: number) { this._current = v; }
...
export function createLiquidSlider(
    ...
    const glass = createLiquidGlass(thumb, {
        width: W, height: H, radius: R,
        bezelWidth: BW, glassThickness: 80, refractiveIndex: 1.45,
        blur: 0, saturation: 7, specularSlope: 0.4, filterMode: 'composite',
        ...glassOpts,
    }) as InternalGlass;
    ...
    ev.add(thumb, 'pointerdown', (e: Event) => {
        ...
        (thumb as HTMLElement).setPointerCapture((e as PointerEvent).pointerId); kick();
    });
    ev.add(window, 'pointermove', (e: Event) => { if (dragging) fromClientX((e as PointerEvent).clientX); });
...
class LiquidCursorHandleImpl extends EventEmitter<LiquidCursorEventMap> implements LiquidCursorHandle {
    readonly element: Element;
    private readonly _cleanup: () => void;
    constructor(element: Element, cleanup: () => void) { super(); this.element = element; this._cleanup = cleanup; }
    destroy(): void { this._cleanup(); this._emit('destroy'); }
}
 ...
 */
export function createLiquidCursor(container: string | Element, options: LiquidCursorOptions = {}): LiquidCursorHandle {
    const cont = resolveEl(container);
    const { size = 90, ...glassOpts } = options;

    if (getComputedStyle(cont).position === 'static') (cont as HTMLElement).style.position = 'relative';
    ...
    const glass = createLiquidGlass(cursor, {
        ...
        bezelWidth: 15, glassThickness: 60, refractiveIndex: 1.45,
        blur: 0.5, saturation: 1.4, specularSlope: 0.8,
        ...glassOpts,
    }) as InternalGlass;
    ...
    let mx = 0, my = 0, inside = false, pressed = false, hovering = false, af: number | null = null;

    function loop() {
        sp.x.setTarget(mx); sp.y.setTarget(my);
    ...
    ev.add(cont, 'mousemove', (e: Event) => {
        ...
        if (!inside) { inside = true; cursor.classList.add('lg-cursor-visible'); sp.x.value = mx; sp.y.value = my; }
        kick();
    });
    ev.add(cont, 'mouseleave',  () => { inside = false; cursor.classList.remove('lg-cursor-visible'); });
    ev.add(cont, 'mousedown',   () => { pressed  = true;  kick(); });
    ev.add(cont, 'mouseup',     () => { pressed  = false; kick(); });
    ev.add(cont, 'mouseenter',  () => { hovering = true;  kick(); });

    return new LiquidCursorHandleImpl(cursor, () => {
        if (af !== null) cancelAnimationFrame(af);
        glass.destroy(); ev.runAll(); cursor.remove();
...
class LiquidInputHandleImpl extends EventEmitter<LiquidInputEventMap> implements LiquidInputHandle {
    ...
    private readonly _cleanup: () => void;
    constructor(element: Element, input: HTMLInputElement, cleanup: () => void) { super(); this.element = element; this._input = input; this._cleanup = cleanup; }
    get value(): string { return this._input.value; }
    set value(v: string) { this._input.value = v; }
    focus(): this { this._input.focus(); return this; }
    blur():  this { this._input.blur();  return this; }
    destroy(): void { this._cleanup(); this._emit('destroy'); }
}
 ...
 */
export function createLiquidInput(target: string | Element, options: LiquidInputOptions = {}): LiquidInputHandle {
    const el = resolveEl(target);
    ...
    const glass = createLiquidGlass(el, {
        width: 340, height: 60, radius: 30,
        bezelWidth: 15, glassThickness: 60, refractiveIndex: 1.5,
        blur: 0.5, saturation: 1.2, specularSlope: 0.7,
        ...glassOpts,
    }) as InternalGlass;
    ...
    function loop() {
        sp.sc.setTarget(1); sp.sx.setTarget(1); sp.sy.setTarget(1);
    ...
    const handle = new LiquidInputHandleImpl(el, input, () => {
        if (af !== null) cancelAnimationFrame(af);
        glass.destroy(); ev.runAll(); input.remove(); el.classList.remove('lg-input-wrapper');
    });

    const ev = makeCleanupTracker();
    ev.add(input, 'focus',  (e) => { sp.sc.setTarget(1.05); kick(); handle._emit('focus', e as FocusEvent); });
    ev.add(input, 'blur',   (e) => { sp.sc.setTarget(1.0);  kick(); handle._emit('blur',  e as FocusEvent); });
    ev.add(input, 'input',  () => {
        sp.sx.velocity += 1.5; sp.sy.velocity -= 0.8;
        kick();
...
class LiquidDialHandleImpl extends EventEmitter<LiquidDialEventMap> implements LiquidDialHandle {
    ...
    private readonly _cleanup: () => void;
    constructor(element: Element, initial: number, cleanup: () => void) { super(); this.element = element; this._angle = initial; this._cleanup = cleanup; }
    get angle(): number { return this._angle; }
    set angle(v: number) { this._angle = v; }
    /** @internal */ _syncAngle(v: number) { this._angle = v; }
    destroy(): void { this._cleanup(); this._emit('destroy'); }
}
 ...
 */
export function createLiquidDial(target: string | Element, options: LiquidDialOptions = {}): LiquidDialHandle {
    const el = resolveEl(target);
    ...
    const glass = createLiquidGlass(knob, {
        ...
        bezelWidth: 20, glassThickness: 80, refractiveIndex: 1.8,
        blur: 0.5, saturation: 1.2, specularSlope: 0.9,
        ...glassOpts,
    }) as InternalGlass;
    ...
    const handle = new LiquidDialHandleImpl(el, initAngle, () => {
        if (af !== null) cancelAnimationFrame(af);
        glass.destroy(); ev.runAll(); knob.remove(); el.classList.remove('lg-dial');
    });
...
class LiquidTooltipHandleImpl extends EventEmitter<LiquidTooltipEventMap> implements LiquidTooltipHandle {
    readonly element: Element;
    private readonly _textEl: HTMLElement;
    private readonly _cleanup: () => void;
    private readonly _show: () => void;
    private readonly _hide: () => void;
    constructor(element: Element, textEl: HTMLElement, show: () => void, hide: () => void, cleanup: () => void) {
        super(); this.element = element; this._textEl = textEl; this._show = show; this._hide = hide; this._cleanup = cleanup;
    }
    show():   this { this._show(); return this; }
    hide():   this { this._hide(); return this; }
    toggle(): this { return this; } // state tracked in closure
    setText(text: string): this { this._textEl.textContent = text; return this; }
    destroy(): void { this._cleanup(); this._emit('destroy'); }
}
 ...
 */
export function createLiquidTooltip(target: string | Element, options: LiquidTooltipOptions): LiquidTooltipHandle {
    const el      = resolveEl(target);
    ...
    const textEl = el.querySelector<HTMLElement>('.lg-tooltip-text') || (() => {
    ...
    const glass = createLiquidGlass(el, {
        ...
        bezelWidth: 15, glassThickness: 60, refractiveIndex: 1.6,
        blur: 0.5, saturation: 1.2, specularSlope: 0.8,
        ...glassOpts,
    }) as InternalGlass;
    ...
    function doShow() { sp.sc.setTarget(1); sp.y.setTarget(0); visible = true; kick(); handle._emit('show'); }
    function doHide() { sp.sc.setTarget(0.5); sp.y.setTarget(20); visible = false; kick(); handle._emit('hide'); }

    const handle = new LiquidTooltipHandleImpl(el, textEl, doShow, doHide, () => {
        if (af !== null) cancelAnimationFrame(af);
        glass.destroy(); ev.runAll(); el.classList.remove('lg-tooltip');
    });
    (handle as LiquidTooltipHandleImpl).toggle = function() { visible ? doHide() : doShow(); return this; };
...
class LiquidProgressHandleImpl extends EventEmitter<LiquidProgressEventMap> implements LiquidProgressHandle {
    readonly element: Element;
    private _val: number;
    private readonly _set: (v: number) => void;
    private readonly _cleanup: () => void;
    constructor(element: Element, initial: number, set: (v: number) => void, cleanup: () => void) { super(); this.element = element; this._val = initial; this._set = set; this._cleanup = cleanup; }
    get value(): number { return this._val; }
    set value(v: number) { this._set(v); }
    /** @internal */ _syncVal(v: number) { this._val = v; }
    destroy(): void { this._cleanup(); this._emit('destroy'); }
}
 ...
 */
export function createLiquidProgress(target: string | Element, options: LiquidProgressOptions = {}): LiquidProgressHandle {
    const el = resolveEl(target);
    ...
    const glass = createLiquidGlass(el, {
        bezelWidth: 12, glassThickness: 50, refractiveIndex: 1.5,
        blur: 0.5, saturation: 1.2, specularSlope: 0.7,
        ...glassOpts,
    }) as InternalGlass;
    ...
    const handle = new LiquidProgressHandleImpl(el, current, (v) => {
        ...
        if (af !== null) cancelAnimationFrame(af);
        glass.destroy(); fill.remove(); el.classList.remove('lg-progress');
    });
