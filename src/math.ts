...
export function compute2D(
    ...
    for (let y1 = 0; y1 < h; y1++) {
        for (let x1 = 0; x1 < w; x1++) {
            const idx = (y1 * w + x1) * 4;
            const x =  x1 < radius       ? x1 - radius
                     : x1 >= w - radius  ? x1 - radius - wB
                     : 0;
            const y =  y1 < radius       ? y1 - radius
                     : y1 >= h - radius  ? y1 - radius - hB
                     : 0;
            const dSq = x * x + y * y;
...
export function computeSpecular(w: number, h: number, radius: number, bezelWidth: number): ImageData {
    const img    = new ImageData(w, h);
